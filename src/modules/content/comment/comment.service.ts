...
export class CommentService {
  ...
  async create(createCommentDto: CreateCommentDto, userId: string): Promise<Comment> {
    ...
    // 更新话题的评论计数
    await this.topicStatsService.incrementCommentCount(topicId);

    // 如果是回复，发送通知给被回复的用户
    if (parentId && replyToUserId && replyToUserId !== userId) {
      await this.notificationTriggerService.notifyCommentReply({
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (comment.topic && comment.topic.commentCount > 0) {
      await this.topicStatsService.decrementCommentCount(comment.topic.id);
    }
  }
