...
export class ReportService {
  ...
  async remove(id: string) {
    ...
    // 删除举报时，减少相关实体的举报计数
    await this.updateTargetReportCount(report.type, report.targetId, -1);
   ...
   * @private
   */
  private async updateTargetReportCount(
    type: ReportType,
    targetId: string,
    delta: number,
  ) {
    try {
      ...
      // 如果目标实体不存在，忽略错误
      console.warn(`Failed to update report count for ${type}:${targetId}`, error);
    }
  }
}
